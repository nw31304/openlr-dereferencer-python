...
from openlr_dereferencer.decoding import configuration, MapObjects, LRDecodeError, DEFAULT_CONFIG
import param
...
class Line(AbstractLine):
    ...
    def end_node(self) -> "Node":
        if type(self.to_int) == Node:
            return(self.to_int)  # type:ignore
        else:
            self.to_int = self.map_reader.get_node(self.to_int)  # type:ignore
            return(self.to_int)
...
class WebToolMapReader(param.Parameterized):
    ...
    def match(self, binstr: str, attempt_fallback: bool = True, clear_cache: bool = True, config: Optional[Config] = None) -> MapObjects:
        ...
        return decode(reference=ref, reader=self, config=config) # type: ignore
